...
import static solutions.mdf_2015.SalesForce.ShortCut.toStrList;
...
public class SalesForce {
    ...
    public static void main(String[] argv) throws Exception {
        ...
        sc.nextLine();
        List<String> pays = toStrList(sc.nextLine(), ";");
        ...
        System.out.println(lignes.stream().collect(Collector.of(() -> new CustomCollector(pays), CustomCollector::accumulate, CustomCollector::combine, CustomCollector::finish)));
    ...
    static class CustomCollector {
        ...
        public CustomCollector(List<String> pays) {
            this.pays = pays;
        }

        void accumulate(String other) {
            if (lignes.contains(Arrays.stream(other.split(";")).limit(3).collect(Collectors.joining()))) {
                doublons++;
            } else {

                if (!Pattern.compile("\\+[0-9]{1,3}-[0-9]{9,11}").matcher(other.split(";")[3]).matches()) {
                    telErr++;
                }
                if (!pays.contains(other.split(";")[4])) {
                    horsZone++;
                }
            }
            lignes.add(Arrays.stream(other.split(";")).limit(3).collect(Collectors.joining()));
        }
        ...
        String finish() {
            return doublons + " " + telErr + " " + horsZone;
        }
