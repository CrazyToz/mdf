...
import static solutions.mdf_2018_14.TourDeControle.ShortCut.*;
...
public class TourDeControle {
    ...
    public static void main(String[] argv) throws Exception {
        ...
        String line;
        sc.nextLine();
        int maxAtt = toInt(sc.nextLine());
        Map<Integer, List<Integer>> att = new HashMap<>();
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("hh:mm");
        while (sc.hasNextLine()) {
            line = sc.nextLine();
            String[] h = line.split(" ")[0].split(":");
            att.computeIfAbsent(toInt(line, 1), ArrayList::new).add(Integer.valueOf(h[0]) * 60 + Integer.valueOf(h[1]));
            /* Lisez les données et effectuez votre traitement */
        }

        Function<Map.Entry<Integer, List<Integer>>, Boolean> func = e -> {
            boolean result = true;
            int start = e.getValue().get(0);
            int diff;
            int acc = 0;
            for (int i = 1; i < e.getValue().size(); i++) {
                diff = e.getValue().get(i) - start;
                acc += diff;
                if (diff < 7 || (acc < 46 && i + 1 > maxAtt)) {
                    result = false;
                    break;
                }
                start = e.getValue().get(i);
            }
            return result;
        };
        if (att.entrySet().stream().map(func).anyMatch(b -> !b)) {
