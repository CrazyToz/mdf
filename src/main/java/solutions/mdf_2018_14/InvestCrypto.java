...
import static solutions.mdf_2018_14.InvestCrypto.ShortCut.toInt;
...
public class InvestCrypto {
    ...
    public static void main(String[] argv) throws Exception {
        ...
        String line;
        sc.nextLine();
        List<Integer> bitc = new ArrayList<>();
        List<Integer> ether = new ArrayList<>();
        while (sc.hasNextLine()) {
            line = sc.nextLine();
            bitc.add(toInt(line, 0));
            ether.add(toInt(line, 1));
            /* Lisez les données et effectuez votre traitement */
        }

        int nbFluctuBitc = 0, nbFluctuEther = 0;
        int prevBitc1 = bitc.get(0), prevBitc2 = bitc.get(1);
        int prevEther1 = ether.get(0), prevEther2 = ether.get(1);
        for (int i = 2; i < bitc.size(); i++) {
            if (prevBitc1 != bitc.get(i) && (bitc.get(i) < prevBitc2 && prevBitc2 > prevBitc1 || bitc.get(i) > prevBitc2 && prevBitc2 < prevBitc1)) {
                nbFluctuBitc++;
            }
            prevBitc1 = prevBitc2;
            prevBitc2 = bitc.get(i);

            if (prevEther1 != ether.get(i) && (ether.get(i) < prevEther2 && prevEther2 > prevEther1 || ether.get(i) > prevEther2 && prevEther2 < prevEther1)) {
                nbFluctuEther++;
            }
            prevEther1 = prevEther2;
            prevEther2 = ether.get(i);
        }

        if (nbFluctuBitc == nbFluctuEther) {
            System.out.println("KO");
        } else if (nbFluctuBitc < nbFluctuEther) {
