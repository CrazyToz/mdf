...
import static solutions.mdf_2016.CorrecteurAutomatique.ShortCut.toInt;

public class CorrecteurAutomatique {

    public static void main(String[] argv) throws Exception {
        Scanner sc = new Scanner(System.in);
        int nbDico = toInt(sc.nextLine());
        ...
        mots.stream().map(mot -> {
            Function<String, Integer> algo = motDico -> new Algo.DamerauLevenshteinAlgorithm(2, 2, 3, 3).execute(mot, motDico);
            return dico.stream().sorted(Comparator.comparing(algo).thenComparing(Comparator.naturalOrder())).findFirst().get();
