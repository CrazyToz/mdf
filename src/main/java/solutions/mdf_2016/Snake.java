...
import static solutions.mdf_2016.Snake.ShortCut.toInt;
...
public class Snake {
    ...
    public static void main(String[] argv) throws Exception {
        ...
        String line;
        Integer tailleS = toInt(sc.nextLine());
        List<Pos> s = IntStream.range(0, tailleS).mapToObj(i -> new Pos(i, 0)).collect(toList());
        sc.nextLine();
        while (sc.hasNextLine()) {
            ...
            switch (line) {
                case "D" -> {
                    Pos queue = s.remove(0);
                    Pos newP = new Pos(s.get(s.size() - 1).x + 1, s.get(s.size() - 1).y);
//                    if (s.stream().noneMatch(p -> p.equals(newP)) && newP.x >= 0 && newP.y >= 0) {
                    s.add(newP);
//                    }
                }
                case "H" -> {
                    Pos queue = s.remove(0);
                    Pos newP = new Pos(s.get(s.size() - 1).x, s.get(s.size() - 1).y - 1);
//                    if (s.stream().noneMatch(p -> p.equals(newP)) && newP.x >= 0 && newP.y >= 0) {
                    s.add(newP);
//                    }
                }
