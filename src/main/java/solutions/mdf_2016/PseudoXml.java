...
public class PseudoXml {
    ...
    public static void main(String[] argv) throws Exception {
        ...
        for (int i = 0; i < line.length(); i++) {
            if (line.charAt(i) == '-') {
                i++;
                balises.add(new Balise(profondeur, line.charAt(i)));
                profondeur--;
            } else {
                profondeur++;
            }
        }
        Map<Character, List<Balise>> result = balises.stream().collect(Collectors.groupingBy(r -> r.nom));
        Comparator<Poids> comparing = Comparator.comparing(poids -> poids.val);
        List<Poids> poids = result.entrySet().stream().map(e -> new Poids(e.getValue().stream().mapToDouble(b -> 1.0 / b.profondeur).sum(), e.getKey())).sorted(comparing.reversed().thenComparing(p -> p.nom)).collect(toList());
        System.out.println(poids.get(0).nom);
    }


    record Balise(int profondeur, char nom) {}

    record Poids(double val, char nom) {}
