...
//        System.err.println("####");
//        List<String> text = Arrays.stream(line.split("\\n")).collect(toList());
        List<String> text = new ArrayList<>();
        while (sc.hasNextLine()) {
            text.add(sc.nextLine());
        }
        Map<String, List<String>> words = text.stream().flatMap(s -> Arrays.stream(s.split("[^a-zA-Z]"))).map(s -> s.toLowerCase()).collect(Collectors.groupingBy(s -> s));
        text.stream().forEach(System.err::println);
        words.entrySet().stream().forEach(System.err::println);
        Comparator<Map.Entry<String, List<String>>> comparingNbWords = Comparator.comparing(e -> e.getValue().size());
        List<String> result = words.entrySet().stream().filter(e -> text.stream().filter(s -> s.contains(e.getKey())).count() != text.size()).sorted(comparingNbWords.reversed().thenComparing(e -> e.getKey(), Comparator.naturalOrder())).limit(3).map(e -> e.getValue().size() + " " + e.getKey()).collect(toList());
