...
import static solutions.mdf_2018_15_30.Carrefour.ShortCut.toStr;
...
public class Carrefour {
    ...
    public static void main(String[] argv) throws Exception {
        ...
        String line;
        sc.nextLine();
        List<Integer> minutesNS = new ArrayList<>();
        List<Integer> minutesEO = new ArrayList<>();
        while (sc.hasNextLine()) {
            line = sc.nextLine();
            int min = Integer.valueOf(line.split(" ")[0].split(":")[0]) * 60 + Integer.valueOf(line.split(" ")[0].split(":")[1]);
            if (toStr(line, 1).equals("N") || toStr(line, 1).equals("S") ) {
                minutesNS.add(min);
            } else {
                minutesEO.add(min);
            }
        ...
        if(minutesNS.stream().map(i -> minutesEO.stream().anyMatch(j -> {
            if (i - j < 3) {
                System.err.println("S " + i + " " + j + " > " + (i - j));
                System.err.println("A " + i + " " + j + " > " + (Math.abs(i - j)));
                System.err.println(Math.abs(i - j) < 3);
            }
