...
import static solutions.mdf_2018_12.SautsPerche.ShortCut.*;

public class SautsPerche {

    public static void main(String[] argv) throws Exception {
        Scanner sc = new Scanner(System.in);
        String line;
        sc.nextLine();
        Map<String, List<Saut>> result = new HashMap<>();
        while (sc.hasNextLine()) {
            line = sc.nextLine();
            result.computeIfAbsent(toStr(line, 0), nom -> new ArrayList<>()).add(new Saut(toDouble(line, 1), toBool(line, "S", 2)));
            /* Lisez les données et effectuez votre traitement */
        }

        Comparator<Saut> comparator = Comparator.comparing(Saut::succes).thenComparing(Saut::h);
        Comparator<Map.Entry<String, List<Saut>>> comparing = Comparator.comparing(e -> e.getValue().stream().max(comparator).get().h);
        List<Map.Entry<String, List<Saut>>> collect = result.entrySet().stream().sorted(comparing.reversed()).limit(2).collect(toList());

        Function<List<Saut>, Optional<Double>> max = sauts -> sauts.stream().filter(Saut::succes).max(Comparator.comparing(Saut::h)).map(Saut::h);
        if (max.apply(collect.get(0).getValue()).orElse(-1.0).equals(max.apply(collect.get(1).getValue()).orElse(-1.0))) {
            Double h = max.apply(collect.get(0).getValue()).orElse(-1.0);
            if (collect.get(0).getValue().stream().filter(saut -> saut.succes && saut.h == h.doubleValue()).count() < collect.get(1).getValue().stream().filter(saut -> saut.succes && saut.h == h.doubleValue()).count()) {
                System.out.println(collect.get(0).getKey());
            } else if (collect.get(0).getValue().stream().filter(saut -> saut.succes && saut.h == h.doubleValue()).count() > collect.get(1).getValue().stream().filter(saut -> saut.succes && saut.h == h.doubleValue()).count()) {
                System.out.println(collect.get(1).getKey());
            } else if (collect.get(0).getValue().stream().filter(s -> !s.succes).count() < collect.get(1).getValue().stream().filter(s -> !s.succes).count()){
                System.out.println(collect.get(0).getKey());
            } else if (collect.get(0).getValue().stream().filter(s -> !s.succes).count() > collect.get(1).getValue().stream().filter(s -> !s.succes).count()){
                System.out.println(collect.get(1).getKey());
            } else {
        ...
        } else if (max.apply(collect.get(0).getValue()).orElse(-1.0) > max.apply(collect.get(1).getValue()).orElse(-1.0)) {
            System.out.println(collect.get(0).getKey());
        } else {
            System.out.println(collect.get(1).getKey());
        }
    ...
    record Saut(double h, boolean succes) {}
